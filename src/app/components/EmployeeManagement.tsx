  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-4 border-t border-border bg-muted/20">
      <div className="flex items-center gap-3 text-[var(--text-sm)] text-muted-foreground w-full sm:w-auto justify-center sm:justify-start whitespace-nowrap shrink-0">
        ...
        <select className="h-11 sm:h-8 px-2 py-0 border border-border rounded-[var(--radius-input)] bg-input-background text-foreground text-[var(--text-sm)] text-center outline-none cursor-pointer hover:bg-muted/50 transition-colors" defaultValue="15" dir="auto">
          <option>10</option><option>15</option><option>30</option>
        </select>
      </div>
      <div className="flex flex-row items-center justify-center sm:justify-end gap-2 pt-2 w-full sm:w-auto">
        <button onClick={() => go(page - 1)} disabled={page <= 1} className="w-11 h-11 sm:w-8 sm:h-8 flex items-center justify-center border border-border rounded-[var(--radius-sm)] bg-card hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer shadow-sm" aria-label="Previous page">
          <ChevronLeft className="w-4 h-4 text-foreground" />
        </button>
        <span className="text-[var(--text-sm)] text-foreground flex items-center gap-2 whitespace-nowrap">
          Page
          <input type="text" value={pageInput} onChange={(e) => setPageInput(e.target.value)} onBlur={() => go(Number(pageInput) || 1)} onKeyDown={(e) => e.key === 'Enter' && go(Number(pageInput) || 1)} className="w-10 h-11 sm:h-8 px-1 py-0 text-center border border-border rounded-[var(--radius-input)] bg-input-background text-foreground focus:ring-2 focus:ring-ring/50 outline-none text-[var(--text-sm)] leading-[42px] sm:leading-[30px] shadow-sm" aria-label="Page number input" dir="auto" />
          <span className="text-muted-foreground">of {totalPages}</span>
        </span>
        <button onClick={() => go(page + 1)} disabled={page >= totalPages} className="w-11 h-11 sm:w-8 sm:h-8 flex items-center justify-center border border-border rounded-[var(--radius-sm)] bg-card hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer shadow-sm" aria-label="Next page">
