...
dom = dom.replace(/أجاز/g, 'اجاز').replace(/اجاز/g, 'اجاز');
i18n = i18n.replace(/أجاز/g, 'اجاز').replace(/اجاز/g, 'اجاز');

// Add "Balance" if it doesn't exist, or replace
if (dom.includes('"Balance":')) {
  dom = dom.replace(/"Balance": ".*?"/, '"Balance": "رصيد الاجازة"');
} else {
  // Add it somewhere
  dom = dom.replace(/"Dashboard":/, '"Balance": "رصيد الاجازة",\n  "Dashboard":');
}
...
const newStr = 'يرجى التأكد من استخدام جميع أيام الاجازة المُرحلة من العام الماضي قبل 31 مارس من هذا العام. تُخصم أيام الاجازة المُرحلة تلقائيًا قبل اجازة هذا العام.';
